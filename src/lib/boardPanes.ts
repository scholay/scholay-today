...
export function parseBoardPanes(raw: string | null, workspace: BoardWorkspace): BoardPanePreference {
  ...
  try {
    ...
    return {
      ...
      listWidth: valid(value?.listWidth, 280, 560, fallback.listWidth),
    };
  } catch { return fallback; }
}
export function fitBoardPanes(viewport: number, preference: BoardPanePreference, hasList: boolean): PaneWidths {
  if (hasList) return fitPaneWidths(viewport, preference.sidebarWidth, preference.listWidth, BOARD_PANE_BOUNDS);
