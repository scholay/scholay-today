...
function loadWorkspace(): Workspace {
  try { const saved = localStorage.getItem(WORKSPACE_KEY); return saved === "hotboard" || saved === "labels" ? saved : "rss"; }
  catch { return "rss"; }
}
...
export default function WorkspaceApp() {
  ...
  const [labelsVisited, setLabelsVisited] = useState(workspace === "labels");
  ...
  const chooseWorkspace = useCallback((next: Workspace) => {
    ...
    if (next === "labels") setLabelsVisited(true);
    setWorkspace(next);
  }, [captureBusy]);
  useEffect(() => {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "1" || event.key === "2" || event.key === "3") {
        ...
        chooseWorkspace(event.key === "1" ? "rss" : event.key === "2" ? "hotboard" : "labels");
      }
    };
  ...
  return <div className="workspace-host">
    <div className="workspace-panels">
      ...
      <div id="workspace-labels-panel" className={`workspace-panel ${workspace !== "labels" ? "is-inactive" : ""}`} role="region" aria-label="标签" aria-hidden={workspace !== "labels"} inert={workspace !== "labels"}>
        {labelsVisited && <LabelBoard active={workspace === "labels"} workspaceSwitch={workspaceSwitch}/>}
      </div>
