...
const hotSources = ["Hacker News", "科技日报", "GitHub Trending"].map((name, n) => ({ id: `preview-${n}`, name, kind: "hot" as const, region: n === 1 ? "china" as const : "global" as const, category: "tech", description: "公开热榜 · 离线界面验收数据", homepage: "https://example.invalid/", project: "预览", project_url: "https://example.invalid/docs", refresh_secs: 600, auth_kind: null, auth_url: null }));
const hotItems = ["研究者如何构建开放的知识网络", "本周值得关注的开源项目", "AI 科学工具的新进展", "从问题出发，走向新的发现"].map((title, n) => ({ id: `hot-${n}`, title, url: `https://example.invalid/hot/${n}`, description: paragraphs[n], rank: n + 1, heat: `${240 - n * 18} 次讨论`, published_at: null }));
let sequence = 0;
...
Object.defineProperty(window, "__TAURI_INTERNALS__", { configurable: true, value: {
  ...
  invoke: async (command: string, args: any = {}) => {
    ...
    if (command === "list_structured_documents") return [];
    if (command === "list_hot_sources") return hotSources;
    if (command === "get_hot_snapshot") return { source_id: args.sourceId, items: hotItems, fetched_at: "2026-09-21T08:00:00Z", last_attempt_at: "2026-09-21T08:00:00Z", status: "ok", error: null, stale: false, cached: true };
    if (command === "get_hot_auth_status") return { configured: false, supported: false };
...
const { useReadingGroups } = await import("../src/lib/readingGroups");
const { LIBRARY_EXAMPLES } = await import("../src/library/examples");
for (const entry of LIBRARY_EXAMPLES.slice(0, 2)) useReadingGroups.getState().openFile(entry.item, true, true);
for (const item of hotItems) useReadingGroups.getState().openHot(hotSources[0], item, "2026-09-21T08:00:00Z", true);
useReadingGroups.setState(s => ({ active: { files: s.tabs.find(t => t.group === "files")!.id, hot: s.tabs.find(t => t.group === "hot")!.id } }));
...
useUi.setState({ mode: new URLSearchParams(location.search).get("theme") === "dark" ? "dark" : "light", palette: "paper", query: { kind: "folder", value: 1 }, queryLabel: "人工智能", sidebarWidth: 220, listWidth: 330, prefs: { ...useUi.getState().prefs, startupView: "last", markReadOnOpen: false, markReadOnScroll: false, hideReadOnStartup: false } });
const tabs = articles.slice(0, 2).map(a => ({ id: `rss-${a.id}-fixture`, articleId: a.id, title: a.title, feedId: a.feedId, restored: true, reading: { ...defaultReadingState(), mode: (a.id === 1 ? "formatted" : a.id === 2 ? "web" : "reader") as "formatted" | "web" | "reader" } }));
...
Object.assign(window, { __RSS_TAB_FIXTURE__: { synthetic: true, status: () => ({ tabs: useReaderTabs.getState().tabs.map(t => ({ id: t.articleId, mode: t.reading.mode })), activeId: useUi.getState().selectedArticleId, nativeCount: pages.size, workspaces: useReadingGroups.getState().tabs.map(t => ({ id: t.id, group: t.group, title: t.title })) }) } });
